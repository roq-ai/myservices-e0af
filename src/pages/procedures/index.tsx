...
import { getProcedures, deleteProcedureById } from 'apiSdk/procedures';
import { ProcedureInterface } from 'interfaces/procedure';

type ColumnType = ColumnDef<ProcedureInterface, unknown>;
...
export function ProcedureListPage(props: ProcedureListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<ProcedureInterface>>(
    () => `/procedures?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: ProcedureInterface) => {
    if (hasAccess('procedure', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/procedures/view/${row.id}`);
    }
  };
...

    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/procedures/view/${record.id}`} passHref legacyBehavior>
            <Button
          ...
          {hasAccess('procedure', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/procedures/edit/${record.id}`} passHref legacyBehavior>
              <Button
                onClick={(e) => e.stopPropagation()}
                mr={2}
                padding="0rem 0.5rem"
          ...
          {hasAccess('procedure', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
            <IconButton
              onClick={(e) => {
                e.stopPropagation();
    ...
    <Box p={4} rounded="md" shadow="none">
      <Flex justifyContent="space-between" mb={4}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="procedure" />
        </Flex>
        {hasAccess('procedure', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/procedures/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
